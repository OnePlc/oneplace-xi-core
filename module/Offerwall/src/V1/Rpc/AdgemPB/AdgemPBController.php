...
class AdgemPBController extends AbstractActionController
{
    /**
    ...
    public function adgemPBAction()
    {
        ...
        if($request->isGet() || $request->isPost()) {
            ...
            if (isset($_REQUEST['authkey'])) {
                $ayetPBKey = $this->mSecTools->getCoreSetting('adgem-pb-key');
                if($ayetPBKey) {
                    $checkKey = filter_var($_REQUEST['authkey'], FILTER_SANITIZE_STRING);
                    if ($checkKey == $ayetPBKey) {
                        $offerWallId = 12;

                        $timeSecret = $this->mSecTools->getCoreSetting('adgem-secret');
                        ...
                        if (is_null($verifier)) {
                            return new ApiProblemResponse(new ApiProblem(422, 'Error: missing verifier'));
                        }
                        ...
                        $calculated_hash = hash_hmac('sha256', $hashless_url, $timeSecret);
                        if ($calculated_hash !== $verifier) {
                            return new ApiProblemResponse(new ApiProblem(422, 'Error: invalid verifier'));
                        }
                        ...
                        if($oCheck->count() == 0) {
                            ...
                            $hash = filter_var($_REQUEST['offer_id'], FILTER_SANITIZE_STRING);
        ...
        return new ApiProblemResponse(new ApiProblem(400, 'Not allowed'));
    }
}
