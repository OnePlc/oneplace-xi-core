...
class NotikPBController extends AbstractActionController
{
    /**
    ...
    public function notikPBAction()
    {
        ...
        if($request->isGet()) {
            ...
            if (isset($_REQUEST['authkey'])) {
                $ayetPBKey = $this->mSecTools->getCoreSetting('notik-pb-key');
                if($ayetPBKey) {
                    ...
                    if ($checkKey == $ayetPBKey) {
                        ...
                        $revenueCheck = filter_var($_REQUEST['amount'], FILTER_SANITIZE_STRING);
                        $pubCheck = filter_var($_REQUEST['pub_id'], FILTER_SANITIZE_STRING);

                        $hashCheck = md5($iUserID. ':'.$revenueCheck.':'.$pubCheck);
                        if($hashCheck != $_REQUEST['signature']) {
                            echo "0";
                            return false;
                        }
                         ...
                         */
                        $txId = filter_var($_REQUEST['txn_id'], FILTER_SANITIZE_STRING);
                        $cWh = new Where();
                        ...
                        if($oCheck->count() == 0) {
                            ...
                            $hash = filter_var($_REQUEST['timestamp'], FILTER_SANITIZE_STRING);
