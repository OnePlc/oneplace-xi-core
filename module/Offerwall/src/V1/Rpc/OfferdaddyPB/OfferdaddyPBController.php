...
class OfferdaddyPBController extends AbstractActionController
{
    /**
    ...
    public function offerdaddyPBAction()
    {
        ...
        if($request->isGet()) {
            ...
            if (isset($_REQUEST['authkey'])) {
                $ayetPBKey = $this->mSecTools->getCoreSetting('offerdaddy-pb-key');
                if($ayetPBKey) {
                    $checkKey = filter_var($_REQUEST['authkey'], FILTER_SANITIZE_STRING);
                    if ($checkKey == $ayetPBKey) {
                        $offerWallId = 11;

                        $apiKeyOfferdaddy = $this->mSecTools->getCoreSetting('offerdaddy-apikey');
                        //Check the signature
                        $my_signature = md5( $_REQUEST['transaction_id']."/".$_REQUEST['offer_id']."/".$apiKeyOfferdaddy);
                        $sig = (isset($_REQUEST['signature'])) ? $_REQUEST['signature'] : '';
                         ...
                         * Check User ID
                         */
                        $iUserID = filter_var($_REQUEST['userid'], FILTER_SANITIZE_NUMBER_INT);
                        ...
                        if($oCheck->count() == 0) {
                            ...
                            $checkRepeat = true;
                            $ayetWhiteList = $this->mSecTools->getCoreSetting('daddy-whitelist-offers');
                            if($ayetWhiteList) {
