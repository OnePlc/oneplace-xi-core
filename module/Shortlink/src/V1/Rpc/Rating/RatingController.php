...
class RatingController extends AbstractActionController
{
    ...
    /**
     * Shortlink Rating Table
     *
     * @var TableGateway $mShortRateTbl
     * @since 1.0.0
     */
    protected $mShortRateTbl;
    ...
    public function __construct($mapper)
    {
        ...
        $this->mShortRateTbl = new TableGateway('shortlink_rating', $mapper);
    ...
    public function ratingAction()
    {
        ...
        if($request->isGet()) {
            ...
            if($retMode == 'def') {
                ...
                $linksToCheck = $this->mShortProviderTbl->select(['active' => 1]);
                foreach($linksToCheck as $sh) {
                    $rated = $this->mShortRateTbl->select(['user_idfs' => $me->User_ID, 'shortlink_idfs' => $sh->Shortlink_ID])->count();
                    if($rated == 0) {
                        $linksDone = $this->mShortDoneTbl->select(['user_idfs' => $me->User_ID, 'shortlink_idfs' => $sh->Shortlink_ID])->count();
        ...
        if($request->isPost()) {
            ...
            $linkInfo = $this->mShortProviderTbl->select(['Shortlink_ID' => $linkId]);
            if(count($linkInfo) == 0) {
            ...
            $rated = $this->mShortRateTbl->select(['user_idfs' => $me->User_ID, 'shortlink_idfs' => $linkId])->count();
            if($rated == 0) {
                # double check that user is allowed to rate the link
                $linksDone = $this->mShortDoneTbl->select(['user_idfs' => $me->User_ID, 'shortlink_idfs' => $linkId])->count();

                if($linksDone > 10) {
                    $this->mShortRateTbl->insert([
                        'user_idfs' => $me->User_ID,
                        'shortlink_idfs' => $linkId,
                        'rating' => $rating,
                        'comment' => $comment,
        ...
        if($request->isPut()) {
            ...
            $ratingFound = $this->mShortRateTbl->select([
                'user_idfs' => $userId,
                'shortlink_idfs' => $linkId
            ]);
            ...
            if($accept == 1) {
                ...
                $this->mShortRateTbl->update([
                    'active' => 1,
                    'verified_by' => $me->User_ID,
                    'verified_date' => date('Y-m-d H:i:s', time())
                ],['shortlink_idfs' => $linkId, 'user_idfs' => $userId]);

                # calculate new overall rating
                $allRatings = $this->mShortRateTbl->select(['shortlink_idfs' => $linkId,'active' => 1]);
                $totalRating = 0;
                $ratingCount = 0;
